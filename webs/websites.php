...
<div class="w3-main" style="margin-right:300px;margin-top:43px;">
    ...
    <header class="w3-container" style="padding-top:22px;">
        <h5><b><i class="fa fa-dashboard"></i>تنظیمات</b></h5>
    </header>
    <div class="w3-white w3-padding-large w3-margin w3-round-medium w3-right" style="width: 80%;">
        <?php
        $ml = new mobile_input();
        $guser = $ml->set_name("user")
            ->set_title("نام کاربری")
            ->set_important(true)
            ->get_str();
        if ($guser != $_SESSION['user']) {
            ?>
        ...
        $fm->alow_visit = false;
        $fm->alow_edit = true;
        $fm->after_edit_url = "main.php";
        $fm->set_tbl_key("website", "user", 0);
        $cuuser = $_SESSION['user'];
        $fm->set_where_edit(" `user`='$cuuser' ");
